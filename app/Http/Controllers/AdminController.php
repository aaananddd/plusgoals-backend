...
class AdminController extends Controller
{
    ...
    public function teachersList()
    {
    	$teachers = User::leftjoin('levels','levels.id','=','users.level_id')
                        ->select('users.first_name','users.last_name','users.created_at', 'levels.level_name')
...
//Task categories
  ...
  //RequestForInternContact list
    ...
    //List employee tasks
    public function ListEmployerTasks(Request $request, $employer_id){

        $result = EmployeeTask::select('task_id', 'task_name', 'task_desc', 'time', 'created_at','created_by')
                                ->where('created_by', $employer_id)
                                ->get();

        foreach ($result as $task) {
            $attachments = EmployeeTaskAttachment::select('id', 'file_name')
            ...
            foreach ($attachments as $attachment) {
                $attachmentData[] = [
                    ...
                    'download_link' => url('download/' . $attachment->id),
                ];
            }

            $task->attachments = $attachmentData;
        }

        if($result == true){
            return view('employer_tasks', ['data' => $result]);
        } else {
            return redirect()->route('GetEmployersList');
        }

    }

    //Employer tasks assigned interns
    public function Assignedinterns(Request $request, $employee_id, $task_id){
        ...
        $result = AssignEmployerTask::leftjoin('employee_tasks', 'employee_tasks.task_id', '=', 'assign_employer_tasks.task_id')
           ...
           if($result != null){
                return view('employer_assigned_interns', ['data' => $result]);
