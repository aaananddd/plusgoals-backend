...
//Download Profile photo
    ...
    public function insertStudentProfile(Request $request, $id)
    {
	   ...
	   if(StudentProfile::where('student_id', $student_id)->exists()){
		 ...
		 $studentDetails = StudentProfile::select('*')->where('student_id', $student_id)->first();
                ...
                $about = $request->about ? $request->about : $studentDetails->about;
                ...
                $about = $request->about ? $request->about : "Accounting graduate with a passion for helping businesses succeed. Seeking an entry-level accounting position where I can use my skills and knowledge to make a positive impact.";
    ...
    public function getStudentDetails()
    {
        ...
        if($role->role == 1){
            $result = Student::leftJoin('student_profiles', 'students.id', '=', 'student_profiles.student_id')
    			->leftJoin('student_details', 'students.id', '=', 'student_details.student_id')  // Ensure this join condition is correct
                ...
                ->select('students.id','students.first_name','students.last_name','students.email_id','students.phone','students.address1','students.address2',
                    'students.city','students.district','students.state','students.district','students.country','students.created_at','students.pincode', 'levels.level_name', 
                    'users.first_name as instructor_first_name', 'users.last_name as instructor_last_name','student_categories.category_name as student_category')  
    ...
    ->select('students.id', 'students.first_name', 'students.last_name', 'students.email_id', 'students.phone', 'students.address1',
        ...
        'student_profiles.experience', 'student_profiles.expected_salary', 'student_profiles.skills','students.city','students.pincode')
    ->where('students.id', $id)
    ->first();
...
if ($result->profile_pic != null) {
	...
	$user_access_token  = $request->token;
        ...
        //    if ($DB_token == $user_access_token) {
                ...
                $result = StudentDetail::where('student_id', $id)->update([
                    'course_id' => $course_id,
                    'level_id' => '1',
                    'mode' => $mode,
                    'difficulty_level' => '1',
	...
	$user_access_token  = $request->token;
	    ...
	    $totalLevels = Level::select('*')->count();
  	    ...
  	    $result[0]['pending_levels']= $pendingLevels;
            if($level[0]->level_status == "Open") {
 	    	$result[0]['levels_attempted']= 0;
            } else {
		$result[0]['levels_attempted']= $level[0]->id;
            }
			...
			return response()->json(['status'=>false, 'message' => "Level not mentioned, Please complete your profile"]);
              }
            } else {
                return response()->json(['status' => false, 'message' => "Invalid token"]);
            }
    ...
    public function getStudentDetailAdminById(Request $request, $id)
    {
        ...
        if(Student::where('id', $id)->exists()){
            $result = Student::leftjoin('student_profiles', 'students.id', '=', 'student_profiles.student_id')
                            ...
                            ->where('students.id', $id)
                            ->orderby('students.id', 'asc')
                            ->get(['students.id as intern_id','students.first_name','students.last_name','students.email_id','students.phone', 'students.address1','students.address2','students.city',
                                   'students.district','students.pincode', 'students.state','students.country',
				'student_details.mode','students.profile_pic','student_tasks.is_completed as student_status','student_tasks.is_completed as student_status', 
		...
		if ($result != null) {
			  if($result[0]->profile_pic != null){
                	$student_profile = $this->downloadImage($result[0]->intern_id);

                	$studentProfileUrl = url('/api/downloadImage/' . $result[0]->intern_id);
                	$result[0]->imageLink = $student_profile;
            	} else {
                	$result[0]->imageLink = null;
            	}
    ...
    public function studentEdit($id){
        ...
        if(Student::where('id', $id)->exists()){
            $result = Student::leftjoin('student_profiles', 'students.id', '=', 'student_profiles.student_id')
                            ...
                            ->where('students.id', $id)
                            ->orderby('students.id', 'asc')
                            ->get(['students.id as intern_id','students.first_name','students.last_name','students.email_id','students.phone', 
				'student_details.mode','student_tasks.is_completed as student_status', 'levels.id as level_id',
                            'levels.level_name', 'difficulty_levels.level_name as difficulty', 'users.first_name as instructor_first_name',
			 'users.last_name as instructor_last_name', 'student_categories.category_name as student_category', 'student_categories.id as student_category_id']);
           ...
           $levels = Level::select('id','level_name')->get();
            ...
            if ($result != null) {
                return view('student_edit', ['data' => array($result), 'instructors' => $instructors, 'category' => $category, 'level' => $levels]);
                return response()->json(['status' => true, 'message' => "Data retreived", 'data' => $result]);
            } else {
                return response()->json(['status' => false, 'message' => "Failed to retreive"]);
            }
    ...
    public function getStudentStatusById(Request $request, $id)
    {
       ...
       $result = Student::leftjoin('student_details', 'students.id', '=', 'student_details.student_id')
            ...
            ->select('students.id','students.first_name','students.last_name','tasks.task_name','tasks.task_desc','tasks.task_id',
                     'levels.level_name', 'difficulty_levels.level_name as difficulty', 'student_tasks.task_status','student_tasks.task_completion','student_tasks.student_answer')
           // ->get(['students.*', 'student_details.*', 'levels.*', 'difficulty_levels.level_name as difficulty', 'tasks.task_name']);
           ->get();

      // $completedTasksCount = $result->where('is_completed', 1)->count(); 
    	...
    	if ($DB_token && is_array($DB_token) && in_array($user_access_token, $DB_token)) {
        	$result = Student::leftjoin('student_profiles', 'students.id', '=', 'student_profiles.student_id')
               ...
               if($task_performance != null){
                ...
                $no_of_tasks_pending = StudentTask::select('*')->where(['student_id'=> $intern_id, 'task_completion' => 0,'level_id' => $task_performance['level_id'], 'difficulty_id'=>$task_performance['difficulty_level']])->count();
...
//Get student details by id for admin panel
   ...
   //Interns view in cv
    public function AdminInternResume(Request $request, $intern_id){
        ...
        if(Student::where('id', $intern_id)->exists()){
               ...
               if($result) {
                   ...
                   if($result != null){
                    ...
                    return view('intern_resume', ['data' => $result]);
                    return response()->json(['status' => true, 'message' => "Data retrieved", 'data' => $result], 200);
                   }
