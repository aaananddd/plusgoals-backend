...
class EmployeeController extends Controller
{
    ...
    ->leftJoin('request_for_intern_contacts', function($join) use ($employee_id) {
        $join->on('request_for_intern_contacts.intern_id', '=', 'students.id')
             ->where('request_for_intern_contacts.employer_id', $employee_id);
    })
    ->leftJoin('student_tasks', 'students.id', '=', 'student_tasks.student_id')
    ->select('students.id', 'students.first_name', 'students.last_name', 'students.created_at', 
             'levels.level_name', 'request_for_intern_contacts.id as request_id',
             'request_for_intern_contacts.is_admin_approved', 
             DB::raw('COUNT(student_tasks.task_id) as task_count'),
             DB::raw("CASE 
                          WHEN request_for_intern_contacts.id IS NULL THEN 'Send Request' 
                          WHEN request_for_intern_contacts.is_admin_approved = 0 THEN 'Request Sent' 
                          WHEN request_for_intern_contacts.is_admin_approved = 1 THEN 'Request Approved' 
                          ELSE 'Unknown Status' 
                       END as request_status"))
    ->where('students.is_active', '1')
    ->where('student_tasks.is_completed', 1)
    ->groupBy('students.id', 'students.first_name', 'students.last_name', 
              'students.created_at', 'levels.level_name', 
              'request_for_intern_contacts.id', 
              'request_for_intern_contacts.is_admin_approved')
...
public function UpdateEmployeeProfessionalDetails(Request $request, $employee_id){
 ...
 //Forgot password
    ...
    public function dashboard(Request $request, $employer_id){
	    ...
	    $profile_completion = Employee::select('is_completed')->where('id', $employer_id)->first();
 ...
 //Get admin approved interns list for employees
    public function ListApprovedInterns(Request $request, $employee_id){
        ...
        if ($DB_token && is_array($DB_token) && in_array($user_access_token, $DB_token)) {
            ...
            $result = Student::leftjoin('request_for_intern_contacts', 'request_for_intern_contacts.intern_id','=','students.id')
                            ->select('students.id', 'students.first_name','students.last_name')
                            ->where(['request_for_intern_contacts.employer_id' => $employee_id, 'request_for_intern_contacts.is_admin_approved' => 1])
                            ->get();

            if($result == true){
                return response()->json(['status'=>true, 'message'=>"Data retreived", 'data'=>$result]);
            } else {
                return response()->json(['status'=>false, 'message'=>"Failed to get data", 'data'=>$result]);
            }
        } else {
            return response()->json(['status'=>true, 'message'=>"Unauthorized"]);
        }
