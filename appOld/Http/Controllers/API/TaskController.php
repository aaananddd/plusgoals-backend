...
//if(Student::where('remember_token', $request->token)->exists()) {
    ...
    if($level->count() > 0) {
        foreach($level as $key => $lvl) {
            ...
            $result[$key]['level_status'] = $lvl->level_status;
            ...
            $tasks = StudentTask::leftjoin('levels','student_tasks.level_id','=','levels.id')
                                    ...
                                    ->where(['student_tasks.level_id'=> $lvl->id,'student_tasks.student_id'=>$student_id])
				    ->distinct()
                                    ->get();
	    ...
	    foreach ($tasks as $task) {
                ...
                if ($task->task_order == 1 || $task->task_order === null) {
                  $task->reattempt = ($attemptCount < 10);
        	} elseif ($task->task_order == 2) {
          ...
          //  $reattempt = ReattemptTask::where([
          //      'intern_id' => $student_id,
           //     'task_id' => $task->task_id
          //  ])->exists() ? false : true;

            // Add reattempt flag to task data
           // $task->reattempt = $reattempt;
          // }
            ...
            $taskCount = StudentTask::where(['student_id' => $student_id, 'level_id' => $lvl->id])->count(); 
            $count = $taskCount;
...
public function checkAnswer(Request $request, $student_id){
    ...
    if (Student::where('id', $student_id)->exists()) {
        ...
        if ($DB_token && is_array($DB_token) && in_array($user_access_token, $DB_token)) {
            $questionsToBeAnswered = Task::select('NofQstnsAns')->where('task_id', $task_id)->first();
            $No_of_answered = $questionsToBeAnswered->NofQstnsAns;
            $result = Question::select('answer', 'id', 'task_type')
            ...
            foreach ($options as $option) {
                $question = $result->where('id', $option['question_id'])->first();
    
                if ($question->task_type == 1) {
                    if ($option['option_id'] == $question->answer) {
                        $correct++;
                    } else {
                        $wrong++;
                    }
                } elseif ($question->task_type == 2) {
                    $student_answer = strtoupper($option['option_id']);
                    $correct_answer = strtoupper($question->answer);
            ...
            StudentTask::where(['student_id' => $student_id, 'task_id' => $task_id])->update([
                'student_answer' => json_encode($options)
            ]);
            ...
            if ($correct >= $No_of_answered) {
                ...
                if (!empty($descriptiveTasks)) {
                    $this->ManualCorrection(new Request([
                        'intern_id' => $student_id,
                        'task_id' => $task_id,
                        'questions' => $descriptiveTasks
                    ]));
                }
...
//         $result = Question::select('answer', 'id')
//                             ->where('task_id', $task_id)
//                             ->whereNotIn('task_type', [3])
//                             ->orderBy('id', 'asc')
//                             ->get();
...
//         foreach ($options as $option) {
//             $question = Question::select('task_type', 'answer')->where(['task_id' => $task_id, 'id' => $option['question_id']])->first();
        
//             // Check if the question type is 1 (multiple choice)
//             if ($question->task_type == 1) {
//                 // If it's multiple choice, compare the option_id with the correct answer
//                 if ($option['option_id'] == $question->answer) {
//                     $correct++;
//                 } else {
//                     $wrong++;
//                 }
//             } elseif ($question->task_type == 2) {
//                 // If it's type 2, compare the answer directly with a, b, c, d
//                 $student_answer = strtoupper($option['option_id']); // Convert to uppercase to match formatting
//                 $correct_answer = strtoupper($question->answer); // Convert to uppercase to match formatting
