    ...
    public function TaskTobeapproved(Request $request, $id){
        ...
        if (Task::where('task_id', $id)->exists()) {
            ...
            if ((!empty($result)) && (!empty($questions))) {
                ...
                return view('approve_tasks', ['data' => array($result), 'question' => ($questions), 'difficultylevel' => $Difficultyresult, 'level' => $levelResult, 'course' => $course]);
   ...
   public function EditQuestions($id, $limit, $currentQuestion)
{
    ...
    if (Task::where('task_id', $id)->exists()) {
        ...
        for($i = $currentQuestion; $i < count($questions); $i ++){
   ...
   public function GetTaskbyIdForEdit($id)
    {
        ...
        if (Task::where('task_id', $id)->exists()) {
            ...
            if ((!empty($result)) && (!empty($questions))) {
                ...
                return view('task_edit', ['data' => array($result), 'question' => ($questions), 'difficultylevel' => $Difficultyresult, 'level' => $levelResult, 'course' => $course, 'attachment'=> $attachmentData, 'taskCategory'=>$taskCategory, 'module'=>$module, 'order'=>$order]);
    ...
    public function GetTaskbyId($id)
    {
        if (Task::where('task_id', $id)->exists()) {
            ...
            if ((!empty($result)) && (!empty($questions))) {
                ...
                //  return view('task_details', ['data' => array($result), 'question' => ($questions)]);
                return view('task_view', ['data' => array($result), 'question' => ($questions), 'difficultylevel' => $Difficultyresult, 'level' => $levelResult, 'course' => $course]);
                //  return response()->json(['status' => true, 'message' => "Data retreived", 'data' =>  $result , 'questions' => $questions]);
            } else if ((!empty($result)) && (empty($questions))) {
	...
	if(Task::where('task_id', $task_id)->exists()) {
        ...
        foreach ($question as $q) {
            ...
            $student_answers = StudentTask::select('student_answer')
                ...
                ->first();
            $decoded_answers = $student_answers ? json_decode($student_answers->student_answer, true) : null;
   ...
   public function getSequentialOrder($task_id, $level_id, $difficulty_id)
{
 ...
 public function GetDescriptiveAnswer(Request $request){
    ...
    $result = ManualCorrection::leftjoin('tasks', 'tasks.task_id','=','manual_corrections.task_id')
                               ->leftjoin('questions','questions.id','=','manual_corrections.question_id')
                               ->leftjoin('students','students.id','=','manual_corrections.intern_id')
                               ->select('tasks.task_id','tasks.task_name','tasks.task_desc','questions.question','manual_corrections.answer','students.first_name',
                                        'students.last_name','manual_corrections.created_at')
                               ->where('tasks.created_by', $instructor)
                               ->get();
...
public function GetDescriptiveById(Request $request, $id){
    ...
    $result = ManualCorrection::leftjoin('tasks', 'tasks.task_id','=','manual_corrections.task_id')
                                ...
                                ->leftjoin('students','students.id','=','manual_corrections.intern_id')
                                ->select('tasks.task_id','tasks.task_name','tasks.task_desc','questions.question','manual_corrections.answer','students.first_name',
                                        'students.last_name','manual_corrections.created_at', 'manual_corrections.id')
